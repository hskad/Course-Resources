...
int main() {
    ...
    cout << "=================================================\n";
    cout << "          ROBUST INTERPOLATION SYSTEM            \n";
    cout << "=================================================\n";

    // 1. INPUT NUMBER OF POINTS
    while (true) {
        cout << "Enter number of data points (2 to " << MAX_POINTS << "): ";
        if (cin >> n && n >= 2 && n <= MAX_POINTS) break;
        
        cout << "Invalid input. Please enter an integer between 2 and " << MAX_POINTS << ".\n";
        cin.clear(); // Clear error flags
        cin.ignore(10000, '\n'); // Discard bad input
    }

    // 2. INPUT DATA POINTS (WITH SORT CHECK)
    ...
    // AUTO-SORT DATA (Handles unsorted input safely)
	sortData(x, y, n);

    // VALIDATION: Check for duplicates
    if (hasDuplicates(x, n)) {
        cout << "\n[CRITICAL ERROR]: Duplicate X values detected!\n";
        cout << "This will cause Division By Zero errors. Please restart.\n";
        return 1;
    }

    // 3. INPUT TARGET
    cout << "\nEnter the target X value to interpolate: ";
    cin >> target;
    ...
    cout << "-------------------------------------------------\n";
    cout << "Target X: " << target << "\n";
    
    // Check if extrapolating to warn user globally
    if (target < x[0] || target > x[n-1]) {
        cout << "[System Warning]: Target is outside the data range (Extrapolation).\n";
        cout << "Some methods may be inaccurate or return errors.\n";
    }
    cout << "\n";

    cout << left << setw(30) << "Method" << "Result" << endl;
    cout << "-------------------------------------------------" << endl;
    
    cout << fixed << setprecision(4);

    cout << left << setw(30) << "1. Piecewise Constant" << interpolatePiecewise(target, x, y, n) << endl;
    cout << left << setw(30) << "2. Nearest Neighbor"   << interpolateNearestNeighbor(target, x, y, n) << endl;
    cout << left << setw(30) << "3. Linear Interpolation" << interpolateLinear(target, x, y, n) << endl;
    cout << left << setw(30) << "4. Newton Divided Diff"  << interpolateNewton(target, x, y, n) << endl;
    cout << left << setw(30) << "5. Lagrange Polynomial"  << interpolateLagrange(target, x, y, n) << endl;
